...
For a function $f: D \to \mathbb{R}$ ($D \subseteq \mathbb{R}^2$), the \textbf{level set} (or level curve) at height $c$ is defined as $\{(x, y) \in \mathbb{R}^2 : f(x, y) = c\}$. A collection of such level curves forms a \textbf{contour map}.
